...
Das Ziel des Scherzugversuches ist es die Maximalkraft $F_{\text{max}}$ zu bestimmen, die die Probe vor ihrem Versagen aushält. Diese Maximalkraft, auch Scherkraft genannt, ist anschließend die oberste Grenze für die Lasthorizonte des nachfolgenden Wöhlerversuches. Um die Beanspruchbarkeit einer Niete zu messen, wird die Scherkraft in der Niete sowie die Lochleibungskraft heran gezogen. Da das vorliegende Bauteil nur eine Scherfläche ($m$=1) und eine Niete ($n_a$=1) besitzt, vereinfacht sich die Formel für die Abscherspannung. Diese setzt sich nach der allgemeinen Definition der Scherspannung in \Eqref{eq:Schubspannung} aus der Scherkraft und dem Nietlochquerschnitt zusammen zu:

    \begin{equation}\label{eq:Abscherspannung_Niete}
        \tau_{\text{a}} = \frac{F}{\frac{\pi \cdot d^2}{4}} = \frac{4 \cdot F}{\pi \cdot d^2}.
    \end{equation}

Die Formel für den Lochleibungsdruck zeigt \Eqref{eq:Lochleibungsdruck_Niete}:

    \begin{equation}\label{eq:Lochleibungsdruck_Niete}
    \sigma_{\text{l}} = \frac{F}{d \cdot t}.
    \end{equation}
    
Dabei ist $d$ der Nietdurchmesser und $t$ die Blechdicke. Sind zwei unterschiedlich dicke Bleche miteinander vernietet, wird mit der Blechdicke des dünneren Bleches gerechnet \cite[Seite 129]{Maschinenteile2007}.  
...
Der arithmetische Mittelwert repräsentiert bei der Versuchsauswertung den Durchschnitt der Maximalkräfte $F_{\text{max}}$~der Proben. Die Standardabweichung zeigt die Streuung der Versuchsergebnisse um den arithmetischen Mittelwert und sagt damit etwas über die Wiederholgenauigkeit der Versuche aus. Der Variationskoeffizient gibt das Verhältnis zwischen der Standardabweichung und dem Mittelwert an und ist eine dimensionslose Kenngröße zum Vergleich zwischen den verschiedenen Probendicken \cite[Seite 23-29]{Statistik2018}. 
	...
	\section{Wöhlerversuch}
       \subsection{Funktionsprinzip Hydropulsanlage}
Für die Erstellung der Wöhlerkurve wird die servohydraulische Prüfmaschine (auch Hydropulser genannt) verwendet. Der Begriff Servohydraulik beschreibt dabei alle hydraulischen Systeme, bei denen geregelte und gesteuerte Volumenströme Anwendung finden \textbf{Quelle für den Satz}  . 
...
Die Einsetzbarkeit einer Hydropulsanlage ist vielseitig. Die Anwendungsbereiche erstrecken sich von Bauteilcharakterisierung über quasistatische und dynamische Bauteilprüfung bis hin zu Vibrations- und Schockprüfung \cite{Instron}. Für diesen Versuch ist allerdings nur die dynamische Bauteilprüfung relevant. 
...
An Sensoren sind im Maschinendynamik-Labor aktuell ein Kraftaufnehmer und ein Wegaufnehmer verbaut und angeschlossen. Beide können zur Regelung der Anlage verwendet werden. Entsprechend der DIN 50100 wird für den vorliegenden Versuch die Kraftregelung angewendet.
Dabei wird die aktuelle Kraft mittels einer Kraftmessdose aufgenommen. Dieses Ist-Signal wird dann gemäß dem Regelkreislauf in \Figref{fig:Regelkreis_Hydropulser_Gesamt} b) verstärkt und an einen PID-Regler weitergegeben, der die Regeldifferenz zwischen dem Ist-Wert und dem Soll-Wert berechnet und in Form eines Stellsignals an das Servoventil weitergibt. Das Signal des Verstärkers kann zusätzlich durch eine Anzeige sichtbar gemacht werden \cite[Seite 262]{Ölhydraulik2020}.
...
Am Ende der Zeitfestigkeitsgerade folgt die Knickschwingspielzahl. Diese liegt im Bereich zwischen $5 \cdot 10^4$ und $10^7$ Schwingspielen und repräsentiert den Übergang von der Zeitfestigkeit zur Langzeitfestigkeit.
...
\end{itemize}
  ...
  Da Typ III nach einmaliger Nennung in der Norm außen vor gelassen wird, wird in \Figref{fig:Vergleich Typ I und Typ II der Wöhlerkurve} nur der Vergleich zwischen Typ I und II veranschaulicht. 
...
Ein zyklisch belastetest Bauteil erfährt einen Dauerbruch. Ursache für einen Dauerbruch ist die Ermüdung. 
	...
	\subsection{Verfahren zur Bestimmung der Zeitfestigkeit}
Zur Bestimmung der Lage der Zeitfestigkeit können laut DIN 50100 zwei Verfahren angewendet werden, die zur Versuchsdurchführung dienen. Im Bereich der Zeitfestigkeit tritt immer ein Ausfall der Probe ein. In dem vorliegenden Fall ist dies ein Nietbruch, aber auch ein Blechriss. 
Das erste der beiden Verfahren ist das Horizontenverfahren. Die Vorgehensweise besteht darin zwei Lasthorizonte zu wählen, die möglichst nah an den Grenzen des Kurzzeit- und des Langzeitfestigkeitbereichs liegen und dort anschließend mehrere Proben zu prüfen (siehe \Figref{fig:Diagramm_Horizontenverfahren}).
...
Der Nachteil des Horizontenverfahren ist, dass es nur dann verwendet werden kann, wenn bekannt ist, wo der Bereich der Kurzzeitfestigkeit und der Langzeitfestigkeit liegt \cite[Seite 273]{Eulitz2022}. 

Da für den vorliegenden Versuch eine geringe Stichprobengröße von insgesamt 15 Proben für den Wöhlerversuch vorhanden sind und zusätzlich der Bereich der Zeitfestigkeit nicht bekannt ist, wird auf das zweite Verfahren zurück gegriffen. 

Das sogennante Perlenschnurverfahren. Das zweite Verfahren ist das Perlenschnurverfahren, welches Verwendung findet, wenn die Lage des Zeitfestigkeitsbereichs nicht definiert ist. Der entscheidende Vorteil ist, dass auch bei einer geringen Probenanzahl eine geeignete Wöhlerlinie bestimmt werden kann. Die Vorgehensweise ist ein Vorantasten auf unterschiedlichen Lasthorizonten, bis zu den Übergängen der Kurz- und Langzeitfestigkeit. Mehrere Proben auf einem Lasthorizont zu prüfen ist möglich \cite[Seite 278]{Eulitz2022}. 
...
Die hervorgehobene Linie kennzeichnet die 50\%-Zeitfestigkeitsgerade mit der Steigung k. Mit der Nummer 1 ist die verschobene Linie dargestellt, die die neue fiktive Schwingspielzahl auf dem fiktiven Lasthorizont darstellt. 
...
In der noch folgenden Versuchsauswertung des Wöhlerversuches wird bei der Erstellung der Wöhlerlinie sowohl die Kraft über die Schwingspielzahl, als auch die Spannungsampflitude über die Schwingspielzahl aufgetragen. Aus diesem Grund kann auch in \Eqref{eq:Zeitfestigkeitsgerade_C} sowie in den folgenden Gleichungen mit der Kraftamplitude gerechnet werden. 

Wird der dekadische Logarithmus auf \Eqref{eq:Zeitfestigkeitsgerade_C} angewendet, kann diese auch wie folgt geschrieben werden:
...
Mit den bestimmten Regressionsparametern können nun die Steigung $k$ und der Lageparameter $C$ der Basquin-Gleichung in \Eqref{eq:reg_C} und berechnet werden. 
...
Unter der Prämisse, dass die Standardabweichung auf allen Lasthorizonten gleich ist (DIN 50100) resultiert, dass die 10$\%$ und 90$\%$ Ausfallwahrscheinlichkeit-Linien parallel zur 50$\%$-Linie angetragen werden können. 
...
Einbezogen werden dürfen in die Zeitfestigkeitsgerade alle Messergebnisse deren Lastamplitude über dem höchsten Durchläufer liegt (DIN 50100).
	...
	\subsection{Verfahren zur Bestimmung der Dauerfestigkeit}
	Zur Bestimmung des Langzeitfestigkeitbereichs schlägt die DIN 50100 verschiedene Verfahren vor. Die Vorgehensweisen folgender Verfahren aus der DIN werden nun erläutert und anschließend das geeignetste Verfahren ausgewählt:
	\begin{itemize}
    \item Probit Verfahren
    \item Abgrenzungsverfahren
    \item Treppenstufenverfahren nach ?
    \item Maximum-Likelihood Methode
	\end{itemize}

Das Probit-Verfahren erfordert eine Festlegung von mindestens 4 Lasthorizonten. Auf jedem Lasthorizont müssen mindestens 10 Proben geprüft werden, um die Streuung auswerten zu können. Wie in \Figref{fig:probit_verfahren} muss dabei jeder Lasthorizont mindestens einen Bruch und einen Durchläufer aufweisen.  

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Probit Verfahren}
