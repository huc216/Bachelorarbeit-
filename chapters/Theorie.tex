...
Für die Erstellung der Wöhlerkurve wird die servohydraulische Prüfmaschine namens Hydropulser verwendet \cite{thkoelln2018}. Der Begriff Servohydraulik beschreibt dabei alle hydraulischen Systeme, bei denen geregelte und gesteuerte Volumenströme Anwendung finden. Ein servohydraulisches System umfasst eine Pumpe mit Antrieb, Leitungen, Ventilen, Aktoren und einen Tank \cite{bfhydraulik}.
...
\Figref{fig:Regelkreis_Hydropulser} veranschaulicht die Funktionsweise mit den erwähnten Bauteilen einer servohydraulischen Prüfmaschine. Zur Differenzierung ist der Ölkreislauf mit durchgezogenen Linien gekennzeichnet und der Regelkreis mit gestrichelten Linien.
Bei dem Stoffkreislauf kommt das Öl aus einem Aggregat, symbolisiert durch eine Pumpe, die das Öl in den Kreislauf pumpt. Das Aggregat umfasst jedoch nicht nur die Pumpe sondern auch den Motor, der die Pumpe antreibt, einen Ölspeicher, sowie einen Hochdruckfilter als auch ein Druckbegrenzungsventil. Das Öl fließt dabei durch ein hochpräzises Servoventil, dass bestimmt, in welche Kammer des Zylinders das Öl fließt und auch wie viel Öl in den Zylinder fließt. Dies ist von Bedeutung, da um den Dreiecksverlauf gefahren wird der Zylinder immer wieder hoch und runter gefahren werden muss. Der Zylinder wirkt dann Kraft auf die Probe aus. 
Verbaut sind im Maschinendynamiklabor aktuell ein Kraftaufnehmer und ein Wegaufnehmer. Beide können zur Regelung der Anlage verwendet werden. Da laut DIN 50100 Kraftregelung für den Wöhlerversuch vorgeschrieben wird, wird sich auch daran gehalten. Zusätzlich kann aber immer auch ein Abbruchkriterium für den Weg gesetzt werden, bei dem die Maschine den Versuch abbricht. 
Der Kraftaufnehmer misst also die aktuelle Kraft mittels einer Kraftmessdose. Dieses Signal wird verstärkt und an einen PID-Regler weiter gegeben, der den Ist-Wert vom Soll-Wert abzieht und dann die Regeldifferenz an das Ventil weiter gibt \cite[Seite 262]{Ölhydraulik2020}. 
...
\begin{tikzpicture}[
    ...
    % Hydraulikversorgung (Aggregat)
    \node[draw, circle, thick, minimum size=1.0cm] (pump) at (6.0, 0.3) {P};
    \node[right=0.4cm] at (pump) {Aggregat};

    % --- VERBINDUNGEN ---
    % Parallele Leitungen Aggregat <-> Ventil
    \draw[line] ([xshift=-0.15cm]pump.north) -- ([xshift=-0.15cm]valve.south); 
    \draw[line] ([xshift=0.15cm]valve.south) -- ([xshift=0.15cm]pump.north);

    % Hydraulik zum Zylinder (Exakt rechtwinkliger Stufenverlauf)
    % Obere Kammer
    \draw[line] ([yshift=0.25cm]valve.west) -- ++(-0.5,0) |- (1.6,2.6); % Vorlauf oben
    \draw[line] (1.6,2.4) -- ++(0.7,0) |- ([yshift=0.1cm]valve.west);   % Rücklauf oben
    
    % Untere Kammer
    \draw[line] ([yshift=-0.25cm]valve.west) -- ++(-0.5,0) |- (1.6,0.4); % Vorlauf unten
    \draw[line] (1.6,0.6) -- ++(0.7,0) |- ([yshift=-0.1cm]valve.west);   % Rücklauf unten

    % Signale
    \draw[signal] (loadcell.east) -- (amp.west);
    \draw[signal] (lvdt.east) -- ++(3.2,0) |- (amp.200); 
...
\end{tikzpicture}
\caption{Aufbau und Regelkreis der Hydropulsanlage (eigene Darstellung in Anlehnung an \cite[Seite 261]{Ölhydraulik2020}).}
\label{fig:Regelkreis_Hydropulser}
\end{figure}


Von den drei zur Verfügung stehenden Hydropulsern im Maschinendynamik Labor an der OTH wird für den vorliegenden Versuch mit dem Hydropulser, der eine Kraft von 16 kN aufbringen kann, gearbeitet. 
...
Bei Nummer 4 in \Figref{fig:Wöhlerkurve} ist die Grenzschwingspielzahl erreicht und die Probe wird, wie bereits erwähnt, als Durchläufer gewertet. Die statische Festigkeit ist an Pfeil 5 vorzufinden.

Hinsichtlich des Aufbaus der Wöhlerkurve lassen sich drei Bereiche unterscheiden.
Im Bereich der Kurzzeitfestigkeit werden die Proben mit einer großen Amplitude beaufschlagt und es ergibt sich eine geringe Lebensdauer. Dabei liegen die Messergebnisse zwischen \(10^{0}\) und \(10^{4}\) Schwingspielen. Die Zeitfestigkeit liegt zwischen  \(10^{4}\) Schwingspielen und der Knickschwingspielzahl $N_K$ und wird durch die Zeitfestigkeitsgerade (siehe Nummer 1 in \Figref{fig:Wöhlerkurve}) beschrieben. Die Zeitfestigkeitsgerade kann entweder mit folgender \Eqref{eq:Zeitfestigkeitsgerade_C} ermittelt werden:

\begin{equation}\label{eq:Zeitfestigkeitsgerade_C}
    N = C \cdot \sigma_{\mathrm{a}}^{-k}.
\end{equation}
 
Eine Alternative Gleichung zur Bestimmung der Zeitfestigkeitsgerade kann unter Verwendung der Knickschwingspielzahl wie folgt nach \Eqref{eq:Zeitfestigkeitsgerade_N_K} aufgestellt werden:

\begin{equation}\label{eq:Zeitfestigkeitsgerade_N_K}
    N = N_{\mathrm{K}} \cdot \left( \frac{\sigma_{\mathrm{a}}}{\sigma_{\mathrm{aL,NG}}} \right)^{-k}.
\end{equation}

Am Ende der Zeitfestigkeitsgerade folgt die Knickschwingspielzahl liegt im Bereich zwischen $5 \cdot 10^4$ und $10^7$ Schwingspielen und repräsentiert den Übergang von der Zeitfestigkeit zur Langzeitfestigkeit.
...
Das erste Verfahren ist das Perlenschnurverfahren, das Verwendung findet, wenn die Lage des Zeitfestigkeitsbereichs nicht bekannt ist. Dabei wird sich mit verschiedenen Lasthorizonten langsam voran getastet. 



Das zweite Verfahren wird Horizontenverfahren genannt. 
Dieses kommt zum Einsatz, wenn bekannt ist, wo der Bereich der Kurzzeitfestigkeit und der Langzeitfestigkeit liegt. Die Vorgehensweise ist dabei 
