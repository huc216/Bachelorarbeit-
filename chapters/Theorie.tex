...
Das erste der beiden Verfahren ist das Horizontenverfahren. Dabei werden zwei Lasthorizonte gewählt, die möglichst nah an den Grenzen des Kurzzeit- und des Langzeitfestigkeitbereichs liegen, um dort anschließend mehrere Proben zu prüfen (siehe \Figref{fig:Diagramm_Horizontenverfahren}).
...
Der Nachteil des Horizontenverfahrens liegt darin, dass es nur dann verwendet werden kann, wenn bekannt ist, wo der Bereich der Kurzzeitfestigkeit und der Langzeitfestigkeit liegt \cite[Seite 273]{Eulitz2022}. 

Das Perlenschnurverfahren findet Verwendung, wenn die Lage des Zeitfestigkeitsbereichs nicht definiert ist. Der entscheidende Vorteil ist, dass auch bei einer geringen Probenanzahl eine geeignete Wöhlerlinie bestimmt werden kann. Die Vorgehensweise ist ein stufenweises Anpassen der unterschiedlichen Lasthorizonte bis zu den Übergängen der Kurz- und Langzeitfestigkeit. Mehrere Proben auf einem Lasthorizont zu prüfen ist möglich \cite[Seite 278]{Eulitz2022}. 
...
Einbezogen werden dürfen in die Zeitfestigkeitsgerade alle Messergebnisse deren Lastamplitude über dem höchsten Durchläufer liegt (DIN 50100).
	...
	Zur Bestimmung des Langzeitfestigkeitbereichs werden in der Fachliteratur und in der DIN~50100 verschiedene Verfahren vorgeschlagen. Die Vorgehensweisen der gängisten Methoden werden nun kurz erläutert, um anschließend das geeignetste Verfahren für diese Arbeit auszuwählen:
	\begin{itemize}
...
Das Treppenstufenverfahren prüft auf einem ersten Lasthorizont eine Probe. Ist diese ein Bruch wird bei der nächsten Probe eine Stufe mit dem festgelegten Stufensprung nach unten gegangen. Ist die Probe hingegen ein Durchläufer wird bei der nächsten Probe um eine Stufe erhöht. 

Der entscheidende Grund, warum das Treppenstufenverfahren schlussendlich ausgewählt wird, ist, dass sich mit diesem Verfahren der Mittelwert am besten ermitteln lässt, da sich die Proben um den Mittelwert einpendeln. 
...
Ausgehend vom untersten Lasthorizont null wird nach oben weiter gezählt. Die Ereignisse Bruch und Durchläufer werden auf jeder Laststufe einzeln zusammen gezählt und fließen in die drei Hilfsgrößen $F_T$ (\Eqref{eq:F_T}), $A_T$ (\Eqref{eq:A_T}) und $B_T$ (\Eqref{eq:B_T}) zur Berechnung des Mittelwertes ein.
