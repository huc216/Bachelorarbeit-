...
Die Versetzungsbewegung tritt ein, wenn die auf das Gleitsystem einwirkende Schubspannung größer ist als die kritische Schubspannung des Werkstoffs~\cite[Seite~88]{Werkstofftechnik-Praktikum2021}. 
...
Zusammengefasst ist die Bewegung von Versetzungen die Grundlage der plastischen Verformung, die Intrusionen und Extrusionen verursacht. An diesen bilden sich Risskeime aus, wobei die Rissinitiierung meist an den Intrusionen einsetzt. Bei der zyklischen Beanspruchung wird der Riss bei jedem Lastwechsel geöffnet und wieder weitestgehend geschlossen, wodurch dieser bei jedem Zyklus wächst~\cite[Seite~267--268]{Werkstofftechnik-Praktikum2021}. 
...
Die Zusammenhänge zwischen den in \Figref{fig:Lastzeitverlaeufe}~Teilbild~a) dargestellten Schwingungskenngrößen werden im Folgenden beschrieben~\cite[Seite~270]{Werkstofftechnik-Praktikum2021}.
...
Eine weitere wichtige Größe ist das Spannungsverhältnis $R$ in \Eqref{eq:Ruhegrad}. Dieses gibt das Verhältnis von Unterspannung zu Oberspannung an:
...
Die dynamischen Lastfälle lassen sich in schwellende und wechselnde Belastung unterteilen. Bei einer schwellenden Beanspruchung behält die Spannung während eines Lastspiels ihr Vorzeichen, während bei einer wechselnden Beanspruchung ein Vorzeichenwechsel zwischen Zug- und Druckspannung auftritt~\cite[Seite 271]{Werkstofftechnik-Praktikum2021}.

Ein Beispiel für eine schwellende Beanspruchung stellt eine 
wiederholt be- und entlastete Feder dar. Eine wechselnde Beanspruchung tritt 
beispielsweise bei einer Pleuelstange in einem Verbrennungsmotor auf~\cite{teclehrerfreund2015}.
...
Bei dem Stoffkreislauf wird das Fluid aus einem Aggregat -- bestehend aus Antriebsmotor mit hydrostatischer Pumpe, Ölspeicher, Hochdruckfilter und Druckbegrenzungsventil -- in das System gefördert. Zur Konditionierung der Druckflüssigkeit sind zudem ein Filter, ein Kühler und ein Vorwärmer integriert~\cite[Seite~3]{Ölhydraulik2020}. Über das elektrohydraulische Servoventil werden die Durchflussmenge und die Richtung des Volumenstroms zum Hydraulikzylinder gesteuert~\cite[Seite~174]{Ölhydraulik2020}. Dadurch wird es möglich, einen Prüfling einer zyklischen Belastung auszusetzen, bei welcher der Zylinder die erforderliche Prüfkraft ausübt~\cite[Seite~262]{Ölhydraulik2020}. 
...
Der Grund dieser Entscheidung besteht darin, dass die Kraftamplitude über die gesamte Versuchsdauer konstant gehalten werden muss~\cite[Seite~9]{DIN50100}. Eine Wegregelung ist für die vorliegende Versuchsdurchführung nicht geeignet. 
Denn bei einer konstant vorgegebenen Wegamplitude würde auch die auf den 
Prüfling wirkende Kraftamplitude abnehmen. Die Probe würde folglich nicht über 
die gesamte Versuchsdauer mit der vorgesehenen Kraftamplitude beansprucht werden, 
wodurch die ermittelte Lebensdauer beeinflusst werden könnte. Aus diesem Grund 
erfolgt die Regelung des Wöhlerversuchs über die Kraft. 

Der zugehörige Regelkreis wird in \Figref{fig:Regelkreis_Hydropulser_Gesamt}~b) gezeigt. Hierbei wird die aktuelle Kraft mittels einer Kraftmessdose aufgenommen. Dieses Ist-Signal wird dann verstärkt und an einen PID-Regler übermittelt, der die Regeldifferenz zwischen dem Ist-Wert und dem Soll-Wert berechnet und in Form eines Stellsignals an das Servoventil weitergibt. Das Signal des Verstärkers kann zusätzlich durch eine Anzeige sichtbar gemacht werden~\cite[Seite~262]{Ölhydraulik2020}. Zur Gewährleistung der Sicherheit können an der verwendeten Hydropulsanlage für den Kraft- und Wegsensor Abbruchgrenzen festgelegt werden. Bei Erreichen einer entsprechenden Kraft- oder Weggrenze wird der Versuch automatisch beendet.
...
Im Gegensatz zu einer klassischen (gaußschen) Normalverteilung basiert eine logarithmische Normalverteilung auf den Logarithmen der betrachteten Größen. So ist beispielsweise nicht die Schwingspielzahl $N$ selbst normalverteilt, sondern ihr dekadischer Logarithmus $\lg N$. 
...
\begin{itemize}
    ...
    \item {Typ II: Gerade mit geringerer Neigung $k_2$}
...
Zu erkennen ist, dass bei Typ II die Spannungsamplitude weiter abfällt, jedoch mit einer geringeren Neigung $k_2$. Im Unterschied dazu verläuft Typ I horizontal ab der Knickschwingspielzahl $N_{\text{K}}$. 
...
Beim sogenannten Horizontenverfahren werden zwei Lasthorizonte gewählt, die möglichst nah an den Grenzen des Kurzzeit- und des Langzeitfestigkeitbereichs liegen, um auf diesen Lastniveaus mehrere Proben bis zum Erreichen des Ausfallkriteriums zu prüfen (vgl. \Figref{fig:Diagramm_Horizontenverfahren})~\cite[Seite~26]{Dissertation_Woehler}.
...
\node[left] at (0,4.5) {$\sigma_{a1}$};
\node[left] at (0,1.8) {$\sigma_{a2}$};
...
Beim Perlenschnurverfahren sind jedoch zwei wesentliche Einschränkungen zu berücksichtigen. Sowohl bei einer zu geringen Probenzahl ($n < 10$) als auch bei einem zu geringen Quotienten der Schwingspielzahlen (Quotient $< 5$) verlieren sämtliche zu schätzenden Kenngrößen (Mittelwert, Standardabweichung und Neigung der Zeitfestigkeitsgeraden) ihre Aussagekraft. Diese erlauben dann keine zuverlässige statistische Aussage über die Grundgesamtheit~\cite[Seite~30]{DIN50100}. 

Für die Standardabweichung der Grundgesamtheit wird in der DIN~50100 ein Bereich zwischen 0,10 und 0,30 angeraten. Wenn keine Referenzwerte vorliegen, wird eine Standardabweichung der Grundgesamtheit von $s_{\lg N,\mathrm{GG}} = 0{,}20$ empfohlen. Dieser Wert stellt lediglich einen Erfahrungswert dar und entspricht daher nicht zwingend der tatsächlichen Standardabweichung der Grundgesamtheit~\cite[Seite~30]{DIN50100}. 
...
Die Parameter $k$ und $C$ in \Eqref{eq:Zeitfestigkeitsgerade_C} stehen für die Neigung der Zeitfestigkeitsgeraden und den Lageparameter der Schwingspielzahl $N$ für $\sigma_{\text{a}} = 1$~\cite[Seite~15]{DIN50100}. 
...
Durch Anwendung des dekadischen Logarithmus wird die Basquin-Gleichung in eine lineare Form überführt. Die logarithmierte Gleichung, die Substitution und die daraus folgende Regressionsrechnung sind in der DIN~50100 angegeben~\cite[Seite~55--56]{DIN50100}:
...
Beide Parameter $a_0$ und $a_1$ werden mit der Methode der kleinsten Fehlerquadrate nach \Eqref{eq:reg_a1} und \Eqref{eq:reg_a0} bestimmt~\cite[Seite~55--56]{DIN50100}: 
...
Mit den bestimmten Regressionsparametern können nun die Neigung $k$ und der Lageparameter $C$ der Basquin-Gleichung in \Eqref{eq:reg_C} und \Eqref{eq:reg_k} bestimmt werden: 
...
\(90\,\%\) entspricht.
Unter der Prämisse, dass die Standardabweichung der logarithmierten Schwingspielzahlen auf allen Lasthorizonten des Zeitfestigkeitbereichs konstant ist, verlaufen die Wöhlerlinien für verschiedene Ausfallwahrscheinlichkeitslinien parallel zueinander~\cite[Seite~63]{DIN50100}. 
...
fiktiven Lasthorizont verschoben (vgl. \Figref{fig:Diagramm_Perlenschnurverfahren})~\cite[Seite~56]{DIN50100}. 
...
Die dabei entstehenden fiktiven Schwingspielzahlen werden gemäß DIN~50100 mit der folgenden \Eqref{eq:N_fiktiv} berechnet~\cite[Seite~56]{DIN50100}:
...
Die Standardabweichung in \Eqref{eq:S_Perlenschnurverfahren} gibt an, wie stark die logarithmierten fiktiven Schwingspielzahlen um den in \Eqref{eq:mittelwert} berechneten Mittelwert streuen~\cite[Seite~56]{DIN50100}:
...
Die ermittelte \(50\,\%\)-Zeitfestigkeitsgerade kann nun parallel durch die auf dem fiktiven Lasthorizont bestimmten Punkte \(N_{10\,\%}\) und \(N_{90\,\%}\) verschoben werden, wodurch die entsprechenden Zeitfestigkeitsgeraden für die beiden Ausfallwahrscheinlichkeiten erhalten werden~\cite[Seite~57]{DIN50100}.

Für die Berechnung der Zeitfestigkeitsgeraden dürfen ausschließlich Versuchsergebnisse verwendet werden, deren Lastamplitude oberhalb des höchsten Lasthorizonts mit einem Durchläufer liegt~\cite[Seite~54]{DIN50100}.

\clearpage

	\subsection{Experimentelle Verfahren zur Bestimmung der Langzeitfestigkeit}\label{chap:Theorie_Treppenstufenverfahren}
...
In dieser Arbeit werden das Probit- und das Abgrenzungsverfahren nur schematisch und überblicksartig vorgestellt, da die DIN~50100 für die Bestimmung der Langzeitfestigkeit das Treppenstufenverfahren ausführlich beschreibt~\cite[Seite~45--49]{DIN50100}. Weitere Gründe für die Wahl des Treppenstufenverfahrens gehen aus dem weiteren Verlauf dieses Kapitels hervor.
...
Beim Treppenstufenverfahren wird der Lasthorizont für die erste Probe zunächst entsprechend einer abgeschätzten Langzeitfestigkeit gewählt. Diese beruht entweder auf Erfahrungswerten oder wird vom Versuchsdurchführenden sinnvoll festgelegt. Stellt die erste Probe einen Bruch dar, wird die Lastamplitude im nächsten Schritt um eine Stufe mit einem definierten Stufensprung reduziert. Erweist sich die Probe hingegen als Durchläufer, erfolgt beim nächsten Versuch eine Erhöhung der Lastamplitude um eine Stufe. Dieses Vorgehen wird analog für jeden Prüfling der gesamten Stichprobe angewendet. Für die Anwendbarkeit des Treppenstufenverfahrens müssen sich auf ein bis drei Laststufen sowohl Brüche als auch Durchläufer befinden. Dieser Zustand stellt sich normalerweise ein, wenn der Stufensprung wie folgt gewählt wird~\cite[Seite 47--49]{DIN50100}: 
...
Nach der Ermittlung des Stufensprungs können die einzelnen Laststufen unter Verwendung einer geschätzten Langzeitfestigkeit $L_{\mathrm{aL},NG}$ gemäß \Eqref{eq:Laststufen} berechnet werden~\cite[Seite~49]{DIN50100}:
...
Ausgehend vom untersten Lasthorizont Null erfolgt die Nummerierung aufsteigend bzw. absteigend. 

Für die abschließende Auswertung der Treppenstufenfolge wird nach dem letzten durchgeführten Versuch eine fiktive Folgeprobe ergänzt. Nach einem Durchläufer wird diese der nächsthöheren Laststufe und nach einem Bruch der nächstniedrigeren Laststufe zugeordnet~\cite[Seite~60]{DIN50100}.

Die mathematische Auswertung des Treppenstufenverfahrens erfolgt nach den Vorgaben der DIN~50100. Dabei werden die Ereignisse Bruch und Durchläufer für jeden Horizont einzeln gezählt und fließen in die drei Hilfsgrößen $F_T$ (\Eqref{eq:F_T}), $A_T$ (\Eqref{eq:A_T}) und $B_T$ (\Eqref{eq:B_T}) zur Berechnung des Mittelwertes ein~\cite[Seite~60--61]{DIN50100}:
...
Hierbei bezeichnet $f_i$ die Anzahl der auswertbaren Versuchsergebnisse auf der Laststufe i. Aus den Hilfsgrößen ergibt sich anschließend der Mittelwert der Langzeitfestigkeit zu \Eqref{eq:LaL_NG}~\cite[Seite~61]{DIN50100}:

\begin{equation} \label{eq:LaL_NG} 
    L_{\text{aL},NG} = L_0 \cdot d_{\lg}^{\left(\frac{A_{\text{T}}}{F_{\text{T}}}\right)}.
\end{equation}

Für die weitere statistische Auswertung wird zunächst die Varianz der Laststufenverteilung bestimmt. Diese ergibt sich nach DIN 50100 zu~\Eqref{eq:D_T}~\cite[Seite~60]{DIN50100}:
\begin{equation}
\label{eq:D_T}
D_{\mathrm{T}} =\frac{F_{\mathrm{T}} \cdot B_{\mathrm{T}} - A_{\mathrm{T}}^{2}}{F_{\mathrm{T}}^{2}}.
\end{equation}

Mit der Varianz $D_{\text{T}}$, dem verwendeten Stufensprung $d_{\lg}$ sowie der Anzahl der auswertbaren Versuchsergebnisse kann anschließend die Standardabweichung der Langzeitfestigkeit $s_{\lg L}$ berechnet werden. Dabei unterscheidet die DIN 50100 zwei Fälle~\cite[Seite~61]{DIN50100}. Für $D_{\mathrm{T}} < 0{,}5$ gilt~\Eqref{eq:s_lgL_1}:
\begin{equation}
\label{eq:s_lgL_1}
...
Zum anderen darf kein Lasthorizont ausschließlich Brüche beziehungsweise Durchläufer enthalten. Ist diese Bedingung nicht erfüllt, empfiehlt die DIN 50100, den verwendeten Stufensprung zu halbieren. Hierzu wird der neue Stufensprung nach~\Eqref{eq:d_lg_1_2} bestimmt und die Laststufen werden entsprechend neu festgelegt~\cite[Seite~60--61]{DIN50100}: 

\begin{equation} \label{eq:d_lg_1_2} 
   d_{\lg,1/2} = \sqrt{d_{\lg}}.   
\end{equation}

	\subsection{Ermittlung der Knickschwingspielzahl}
	Aus der zuvor bestimmten Zeitfestigkeitsgeraden kann die Knickschwingspielzahl $N_K$ gemäß \Eqref{eq:N_K} bestimmt werden~\cite[Seite~62]{DIN50100}:
...
Bei einer Wöhlerlinie des Typs~I verläuft der Langzeitfestigkeitsbereich horizontal, sodass die Lastamplitude unabhängig von der Schwingspielzahl konstant bleibt. Es gilt \Eqref{eq:La_TypI}~\cite[Seite~105]{DIN50100}:

\begin{equation} \label{eq:La_TypI} 
    L_{\text{a}} = L_{\text{aL},NK}.
\end{equation}

Bei einer Wöhlerlinie des Typs~II nimmt die Lastamplitude auch im Langzeitfestigkeitsbereich mit zunehmender Schwingspielzahl weiter ab. Die Lastamplitude ergibt sich in diesem Fall zu \Eqref{eq:La_TypII}~\cite[Seite~105]{DIN50100}:

\begin{equation} \label{eq:La_TypII} 
    L_{\text{a}} = L_{\text{aL},NK} \cdot \left(\frac{N}{N_{\text{K}}}\right)^{\frac{1}{-k_2}}.
\end{equation}

Dabei bezeichnet \(k_2\) die Neigung der Wöhlerlinie im Langzeitfestigkeitsbereich und beschreibt den weiteren Abfall der Lastamplitude bei Wöhlerlinien des Typs II~\cite[Seite~105]{DIN50100}.
