...
\clearpage
	...
	Zur Bestimmung des Langzeitfestigkeitsbereichs werden in der DIN~50100 verschiedene Verfahren vorgeschlagen. Nachfolgend werden die gängigsten Methoden erläutert, um anschließend das für diese Arbeit adäquateste Verfahren auszuwählen:
	\begin{itemize}
...
In \Figref{fig:probit_verfahren} ist zu erkennen, dass die Ausfallwahrscheinlichkeit $P_A$ auf jedem Lasthorizont separat ausgewertet wird.  
...
Der maßgebliche Vorteil des Treppenstufenverfahrens besteht darin, dass sich mit dieser Methode der Mittelwert präzise und ohne großen Aufwand ermitteln lässt, da sich die Versuchsergebnisse systematisch um den Mittelwert einstellen. 

Während im Zeitfestigkeitsbereich nach Vorgabe der Lastamplitude die resultierende Bruchschwingspielzahl als Antwort des Systems gemessen wird und sich somit im Wöhlerdiagramm eine Streuung in horizontaler Richtung (d.~h. in Richtung der Schwingspielzahl-Achse) ergibt, wird im Langzeitfestigkeitsbereich das Ziel verfolgt, die Amplitude mit $50\,\%$ Ausfallwahrscheinlichkeit zu bestimmen. Daher breitet sich die Streuung in diesem Fall in vertikaler Richtung (d.~h. entlang der Lastamplituden-Achse) aus.

Ist die Standardabweichung im Vorhinein nicht bekannt, ist auf empirisch ermittelte Werte zurückzugreifen. Die folgende \Tabref{tab:Standardabweichung_Treppenstufenverfahren} enthält die Standardabweichung der Grundgesamtheit und den korrespondierenden Stufensprung $d_{\lg}$ für verschiedene Materialien:
...
Für weitere Materialien und deren Werte wird auf die Tabelle in Anhang C der DIN~50100 verwiesen. 
...
Anhand der Ermittlung des Stufensprungs, können unter ZuHilfenahme einer geschätzten Langzeitfestigkeit $L_{\text{aL},NG}$ die Laststufen anhand von \Eqref{eq:Laststufen} berechnet werden.
\begin{equation} \label{eq:Laststufen} 
    L_i = L_{\text{aL},NG} \cdot (d_{\lg})^i
\end{equation}

Die DIN~50100 legt dabei nahe sich vor Versuchsbeginn bis zu zehn Laststufen zu ermitteln.
